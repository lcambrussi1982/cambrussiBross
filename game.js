(() => {
  // Cambrussi Bross — PRO + mobile + fogo (H) + 3 fases
  ...
  // C moeda | E inimigo | F bandeira | P spawn | H fogo (perigoso, não-sólido)
  const SOLIDS = new Set(['X','=', 'B', '?', 'Q']);
  ...
  if (tcButtons.length){
    ...
    // Evita scroll/zoom nos controles
  ...
  const ASSETS = {
    images:{
      ...
      // fogo será desenhado por código (sem sprite)
    },
    sfx:{
    ...
    constructor(raw){
      ...
      for (let r=0;r<this.h;r++){
        for (let c=0;c<this.w;c++){
          ...
          else if (ch==='H') this.hazards.push(new Fire(x+10, y+TILE-6)); // base sobre o chão
          else if ('X=B?Q'.includes(ch)) this.setTile(c,r,ch);
        }
      }
    }
    ...
    drawTiles(cam){
      ...
      for (let r=r0;r<=r1;r++){
        for (let c=c0;c<=c1;c++){
          ...
          let img=IM.ground; if (t==='=') img=IM.platform; else if (t==='B') img=IM.brick; else if (t==='?') img=IM.qblock; else if (t==='Q') img=IM.qused;
  ...
  // ===== Fogo / Perigo (não sólido) =====
  class Fire{
    // x,y = base do fogo (perto do chão)
    constructor(x,y){ this.x=x; this.y=y; this.t=0; this.w=28; this.h=28; }
    aabb(){
      // chama "pulsante"
      const h = this.h + Math.sin(this.t*8)*4;
      return { x:this.x-10, y:this.y - h, w:20, h:h };
    }
    update(dt){ this.t += dt; }
    draw(cam){
      ...
      for (let i=0;i<2;i++){
        const sx = bx + (i? -6: 6);
        const sy = by - 20 - (Math.sin(this.t*10+i)*6);
  ...
  // ===== Entidades =====
  class Player{
    constructor(x,y){ this.x=x; this.y=y; this.w=32; this.h=42; this.vx=0; this.vy=0; this.onGround=false; this.frame=0; this.animT=0; this.facing=1; this.score=0; this.deaths=0; this.won=false; this.spawn={x,y}; }
    ...
    update(dt, world){
      ...
      if (headBump){
        ...
        if (tile==='?'){ world.setTile(c,r,'Q'); floatingCoins.push(new FloatingCoin(c*TILE+TILE/2-6, r*TILE-6)); this.score+=1; play('coin'); }
        else if (tile==='B'){ world.setTile(c,r,'.'); breakParticles.push(new BreakParticle(c*TILE, r*TILE)); play('break'); }
      }

      // Limites & queda
      this.x=clamp(this.x,0,world.w*TILE-this.w);
      if (this.y>world.h*TILE+TILE*2){ this.deaths++; this.resetToSpawn(); }
      ...
      for (const e of world.enemies) if (e.alive && rectsIntersect(this.aabb(),e.aabb())){
        ...
        else { this.deaths++; this.resetToSpawn(); }
      }

      // **FOGO**: toca -> dano
      for (const h of world.hazards){
        if (rectsIntersect(this.aabb(), h.aabb())){ this.deaths++; this.resetToSpawn(); break; }
      }
  ...
  function drawHUD(p,idx){ hud.innerHTML=`<strong>Cambrussi Bross</strong> · Fase: ${idx+1}/${LEVELS.length} · Moedas: ${p.score} · Mortes: ${p.deaths}`; }

  let world, player, levelIndex=0;
  function loadLevel(i){ const raw=LEVELS[i]; world=new World(raw); player=new Player(world.spawn.x,world.spawn.y); }
  ...
  async function start(){
    ...
    function frame(now){
      ...
      if (keys.has('r')){
        player.score=0; player.deaths=0; player.won=false; hideOverlay(); player.resetToSpawn();
        for (const c of world.coins) c.taken=false; for (const e of world.enemies) e.alive=true;
      }
      ...
      if (!player.won){
        ...
        if (wasPressed.size>0){ hideOverlay(); levelIndex++; if (levelIndex>=LEVELS.length){ levelIndex=0; showOverlay(`Parabéns! Você zerou <em>Cambrussi Bross</em>!<small>Toque ↻ ou pressione R para recomeçar</small>`); } else loadLevel(levelIndex); player.won=false; wasPressed.clear(); }
      }
